...
### W4ER 的已知盲點（必須跟指標本身一起記住）

W4ER 測的是**「使用者是否還在意這個 UI 元件」**，不是「使用者是否還信任判決書的內容」。
這兩件事會在兩個方向上分歧：

| 情境 | W4ER | 真實狀況 |
|---|---|---|
| 團隊真的照著建議修法動手改 | **偏低** | 看到 → 理解 → 去修，是比點讚更自然的下一步。點讚是額外的 meta 行為 |
| 團隊已半放棄，隨手滑過 Slack | **偏高** | 對按鈕的反射性互動不代表讀了內容 |

因此：

1. **W4ER 永遠與「判決書有用率」和「根因命中率」交叉分析，不單獨解讀。**
   W4ER 高但有用率低 = 反射性點擊，是壞訊號，不是好訊號。
2. 「根因命中率」（人工抽樣）雖然貴且不能自動化，但它才是**最貼近北極星精神**的指標。
   它被列為次要只是因為取得成本高，不是因為重要性低。
3. Phase 4 起，`/stats` 必須同時呈現這三個數字，不能只給 W4ER。
...
- repo 內 `.prosecutor.yml` 設定檔
  （⚠️ **private repo 的可行性、以及會影響他人的欄位是否該留在 repo 內，
  取決於 `RISKS.md` Q2 的拍板結果**。不要只讀本節就開始設計）
...
║                                                                               ║
║  ⑩ ★ 先落庫 ★（交易內）：cases（含配發的 case_no）/ meme_cards / llm_usage    ║
║     皆為遮罩後內容。此時 case_id 已存在 → 回饋按鈕的外鍵必定有效              ║
║                                                                               ║
║  ⑪ 投遞 Slack Block Kit 判決書 + 回饋按鈕 + 迷因卡                            ║
║                                                                               ║
║  ⑫ 回填 cases.slack_channel_id / slack_ts                                     ║
║     events.status = 'done' | 'failed'（失敗留在死信區，可重跑投遞而不重生內容）║
╚═══════════════════════════════════════════════════════════════════════════════╝
                    │                                          │
                    ▼                                          ▼
        ┌───────────────────────┐                  ┌───────────────────────┐
...
看起來反直覺——投遞才是使用者感受得到的事，落庫只是記帳。但順序反過來會產生一個
**靜默腐蝕北極星指標**的故障：

Slack 訊息上必然帶著 `case_no`（品牌元素）與回饋按鈕，而按鈕的 payload 必須帶
`case_id` 才能寫進 `feedback` 表（該表有 `case_id NOT NULL REFERENCES cases(id)`）。
若先投遞、後落庫，而 worker 在兩者之間崩潰或落庫失敗，使用者手上就有一則
看起來完全正常、案號齊全的判決書，但按下 👍/👎 時外鍵約束會失敗。

這種例外極可能被某層 `try/catch` 吞掉（它是邊角案例，沒人會為它寫測試），
使用者感受到的是「點了沒反應」——比沒收到判決書更傷信任。而且它會讓 W4ER
（§2 的北極星）的分子**系統性偏低而無人知曉**：我們會以為產品被冷落，
其實是我們自己弄丟了點擊。

因此：**`cases` 與 `meme_cards` 在同一個交易內先建立，`case_no` 當場配發，
投遞成功後才 UPDATE `slack_ts`。** 投遞失敗時 `case_id` 依然存在，
可以重試投遞而不必重新呼叫 LLM 與重新渲染（順帶省錢）。
...
#### 24h 快取命中時，只省略「呼叫 LLM」這一步

`verdict_cache` 命中（同 `signature_hash` + installation + tone + locale）時，
**以下全部必須對「本次」事件重新求值，不得沿用快取當時的結果**：

| 項目 | 理由 |
|---|---|
| §7.5 出站健檢 | 便宜，且沒有理由跳過 |
| §7.2 confidence 伺服器端夾制 | 本次的 `redaction_ratio` / `compression_level` / `signature_found` **與快取產生時不同** |
| §7.3 severity 計算 | 本次的分支、workflow、`repeat_tier` 都可能不同 |
| D-12 的遮罩率門檻（> 40% 不出稿） | **這條最重要**：同一個 `signature_hash` 可能來自不同 job、不同 log，本次遮罩率 55% 就該走 fallback，不能因為 24h 內命中快取就照樣把舊文案發出去 |
| §8.3 場景選擇 | 輸入特徵是本次的 |
...
#### 第一道：渲染層——LLM 自由文字一律 `plain_text`

> **這是主要控制項，不是輔助措施。**

Slack 的 `mrkdwn` 文字物件會**自動偵測並連結裸網域**——`ci-fix-helper.example.net/patch`
這種沒有 `https://` 前綴的字串，不需要任何 markup 就會變成可點擊連結。
任何基於「過濾 `https?://`」或「跳脫 `<`」的防線對這個變體**完全無效**。

因此：

| 內容 | Block Kit 型別 |
|---|---|
| **所有 LLM 產生的自由文字**（`charge` / `evidence` / `root_cause_hypothesis.*` / `recommended_fix.statement` / `meme_title`） | `type: "plain_text"`，`emoji: false` |
| 產品自有的固定框架（標題列、分隔線、欄位標籤、按鈕、頁尾連結） | 可用 `mrkdwn` |
| `recommended_fix.file` / `line` | `plain_text`，且先驗證為相對路徑形態 |

`plain_text` 不解析 markup、不自動連結、不展開 `<!channel>`。這一步一次消滅
釣魚連結、mention 轟炸、與 mrkdwn 注入三類攻擊，且**不需要維護 TLD 清單**——
維護清單的方案必然會漏，這個方案結構上不會。

代價：判決書內文不能有粗體與行內連結。可接受——公文本來就不該有。

**分享頁（HTML）對應規則**：`meme_title` 與任何 LLM 文字進入 HTML 前一律
HTML-escape，且 CSP 禁止 inline script 與外部資源。

#### 第二道：欄位健檢（防禦縱深）

即使有第一道，以下檢查仍然全部執行——攻擊者只要突破其中一層就贏，我們必須每層都守：

| 檢查 | 動作 |
|---|---|
| `defendant_ref` 是 enum 值 | 違反即 fallback |
| 任何字串欄位含 scheme URL（`https?://`、`ftp://`、`data:`、`javascript:`） | 剝除 |
| 任何字串欄位含**裸網域形態**（`[a-z0-9-]+(\.[a-z0-9-]+)+/` 或結尾為常見 TLD） | 剝除。`recommended_fix.file` 例外（相對路徑白名單，且不得含 `/` 以外的分隔或 `..`） |
| 任何字串欄位含 `@` 開頭 mention、`<!channel>` / `<!here>` / `<!subteam^…>` | 剝除 |
| 含 `[REDACTED:` 以外形態的疑似秘密（重跑輕量 sanitizer 掃描） | 命中即 fallback，記為 **P0 告警** |
| 字串長度上限 | 超出即截斷 |

> 第二道的裸網域規則刻意比第一道寬鬆（允許 `client.ts` 這種含點的檔名），
> 因為第一道已經讓「就算漏掉也不會變成連結」成立。兩道的嚴格度不同是設計，不是疏漏。
...
**但窮舉不是正確性的主要論證。** 因為表是 first-match-wins，一旦
`confidence == insufficient` 命中優先序 10，其餘 8 個特徵的 704 種組合
全部收斂到同一個 `SC10`——窮舉在這裡只是把同一條規則重跑 704 次。
它驗證的是「不會掉進未定義行為」，不是「規則寫對了」。

因此驗收標準分兩層：

| 層 | 測試 | 驗證什麼 |
|---|---|---|
| **主** | 每條優先序規則各至少一組**正例**（應命中）+ 一組**邊界對照組**（只差一個特徵就不該命中） | 規則寫對了、優先序排對了 |
| **輔** | 笛卡兒積窮舉 | 全覆蓋、無未定義行為、輸出恆為單一場景 |
...
| **輸入** | 標題生成的 LLM 呼叫**只**收到已通過 schema 驗證與 §7.5 健檢的公訴詞 JSON，**絕不**收到錯誤區段原文 |

#### R7 的技術落實（不能只靠 ASCII token 檢查）

原本的規則是「標題中的識別符 token（`[A-Za-z_][A-Za-z0-9_.]{2,}`）必須全部出現在來源中」。
這條規則對**預設語系 `zh-TW` 幾乎是空判斷**：一個完全用中文編造的新指控——
例如「隔壁工程師蓄意搞砸」——裡面一個 ASCII token 都沒有，驗證迴圈對零個 token
恆為真。R7 在 `CLAUDE.md` 是鐵則等級，不能只對英文有效。

四層檢查，任一層不過即改用確定性模板標題：

| 層 | 檢查 | 說明 |
|---|---|---|
| **T1 輸入隔離** | 標題 LLM 呼叫的輸入只有已驗證的公訴詞 JSON | 注入必須先突破公訴詞的 schema 才有機會影響標題。這是最有效的一層 |
| **T2 語彙拒絕清單** | 標題不得含人稱指涉或動機指控：`你/妳/他/她/某人/同事/隔壁/大家/誰`、`蓄意/故意/惡意/明知/擺爛/擺明` 等 | 對事不對人（R5）。清單存成資料，與場景規則同樣可測試、可擴充 |
| **T3 來源涵蓋率** | 中文：標題的 CJK 2-gram 至少 **60%** 出現在來源文字聯集中<br>英文/ASCII：識別符 token **100%** 必須出現 | 60% 而非 100%，是因為濃縮必然引入連接詞與句式。這個門檻在 Phase 3 用 10 個 golden case 校準 |
| **T4 保底** | `confidence == "insufficient"` 時**一律**使用模板標題，不呼叫 LLM | 證據不足的案子最沒有素材可濃縮，也最容易讓模型自由發揮 |

來源文字聯集 = `charge` + `evidence[]` + `root_cause_hypothesis.statement` + `recommended_fix.statement`。**不含**錯誤區段原文（呼應 T1）。

> **這裡我保留了 LLM 生成標題**（`THREAT_MODEL.md` §5.6 建議改成純機械組合，那更安全）。
> 理由：機械組合的標題會很難看，而迷因卡是傳播引擎，難看等於失效。
> T1 + T4 已經把最危險的兩個情境（注入、無素材）關掉，T2 + T3 處理剩下的。
> 若 Phase 3 的 redteam 能穩定突破 T2/T3，就退回機械組合——這是預先講好的撤退線。
